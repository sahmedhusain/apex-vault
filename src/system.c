...
void checkAccountDetails(struct User u)
{
    ...
    while (getAccountFromFile(pt, userName, &r))
    {
        if (r.id == accountId && r.userId == u.id)
        {
            ...
            double rate = 0.0;
            if (strcmp(r.accountType, "saving") == 0) rate = 0.07;
            else if (strcmp(r.accountType, "fixed01") == 0) rate = 0.04;
            else if (strcmp(r.accountType, "fixed02") == 0) rate = 0.05;
            else if (strcmp(r.accountType, "fixed03") == 0) rate = 0.08;
            else if (strcmp(r.accountType, "current") == 0) rate = 0.0;
    ...
    while (getAccountFromFile(pt, userName, &r))
    {
        if (r.id == accountId)
        {
            if (r.userId == u.id) {
                ...
                // For now, just assume yes as per instructions not explicitly requiring strict user check logic here.
                // NOTE: We do NOT have the new owner's username here. 
                // records.txt requires a username field. 
                // If we update userId but not username, consistency is broken?
                // `records.txt` format: id userId name ...
                // The `name` field in records.txt seems to be the User's name.
                // If I transfer ownership, I should update the name too.
